...
export default function TurnRegister({ currentUser, onLogout }) {
  ...
  const [ciForm, setCiForm] = useState({
    room: '', guest: '', type: 'in', time: '', checks: makeChecks('in')
  });
  const [invForm, setInvForm] = useState({
    number: '', guest: '', amount: '', method: 'efectivo'
  });
  const [incForm, setIncForm] = useState({
    type: 'alojamiento', concept: '', amount: '', method: 'efectivo'
  });
  const [expForm, setExpForm] = useState({
    concept: '', amount: '', category: 'operativo'
  });
  ...
  }, [userName, shiftData, detectedShift, cajaMenorSaldo, sessionId, sessionStartTime]);
  ...
  const handleLogout = async () => {
    if (!window.confirm('¿Cerrar sesión?\n\nSi no entregaste el turno, los datos se guardarán para continuar después.')) return;
  ...
  const delExpense = async (id) => {
    ...
    setShiftData(prev => ({
      ...prev,
      cajaMenor: { ...prev.cajaMenor, expenses: expenses.filter(e => e.id !== id) }
    }));
  };

  const delCheckin = (id) => setShiftData(prev => ({
    ...prev, checkins: (prev?.checkins || []).filter(c => c.id !== id)
  }));

  const delInvoice = (id) => setShiftData(prev => ({
    ...prev, invoices: (prev?.invoices || []).filter(i => i.id !== id)
  }));

  const delIncome = (id) => setShiftData(prev => ({
    ...prev,
    cajaMayor: {
      ...prev.cajaMayor,
  ...
  const handleCloseShift = async () => {
    ...
    try {
      ...
      await generateShiftHandoverPDF(turnoData, detectedShift, userName, cajaMenorSaldo);
    ...
    <div className="tr-root">
      <header className="tr-header">
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          {detectedShift === 'night' && (
            <button className="btn-night-audit" onClick={() => setShowNightAudit(true)}>
              <FaFileAlt /> Auditoría
            </button>
          )}
          ...
          <div className="caja-card caja-menor">
            ...
            {!cajaMenorLocked ? (
              ...
              <div className="saldo-bloqueado">
                ...
                <button
                  style={{ background: 'none', border: 'none', color: '#6a4a9a', cursor: 'pointer', marginLeft: 3 }}
              ...
              shiftData.checkins.map(ci => {
                ...
                return (
                  <div key={ci.id} className="tr-item">
                    ...
                    <button className="btn-del" onClick={() => delCheckin(ci.id)}><FaTrash /></button>
                  </div>
                );
              })
            )}
              ...
              cajaMayorIncome.map(inc => {
                ...
                return (
                  <div key={inc.id} className="tr-item">
                    ...
                    <button className="btn-del" onClick={() => delIncome(inc.id)}><FaTrash /></button>
                  </div>
                );
              })
            )}
            ...
            {(!shiftData?.invoices || shiftData.invoices.length === 0) ? (
              ...
              shiftData.invoices.map(inv => (
                <div key={inv.id} className="tr-item">
                  ...
                  <button className="btn-del" onClick={() => delInvoice(inv.id)}><FaTrash /></button>
                </div>
              ))
            )}
          </div>
              ...
              cajaMenorExpenses.map(exp => (
                <div key={exp.id} className="tr-item">
                  ...
                  <div className="tr-item-body">
                    <div className="tr-item-title">
                      ...
                      {exp.addedBy && exp.addedBy !== userName && (
                        <span style={{ fontSize: '0.75rem', opacity: 0.5, marginLeft: 6 }}>(por {exp.addedBy})</span>
                      )}
                    </div>
                  ...
                  <button
                    ...
                    title={exp.addedBy && exp.addedBy !== userName ? `Solo ${exp.addedBy} puede eliminar` : ''}
                  >
                    <FaTrash />
                  </button>
                </div>
        ...
        <div className="tr-panel tr-full">
          ...
          <div className="entrega-info">
            ...
            <div className="ei-item">
              ...
              <span className="ei-val" style={{ color: '#4ade80' }}>{fmt(totalEfectivo)}</span>
            </div>
            <div className="ei-item">
              <span className="ei-label">Tarjeta</span>
              <span className="ei-val" style={{ color: '#60a5fa' }}>{fmt(totalTarjeta)}</span>
            </div>
            <div className="ei-item">
              <span className="ei-label">Transferencia</span>
              <span className="ei-val" style={{ color: '#a78bfa' }}>{fmt(totalTransferencia)}</span>
            </div>
            <div className="ei-item">
