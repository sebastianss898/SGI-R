...
import {
  FaTools, FaPlus, FaEdit, FaTrash, FaClock,
  FaCheckCircle, FaExclamationTriangle, FaUser,
  FaCalendarAlt, FaFilter, FaSearch, FaBed,
  FaSpinner, FaClipboardList, FaWrench, FaCheck,
  FaCog
} from 'react-icons/fa';
...
const LOCATION_TYPES = {
  ...
  area_comun: { label: 'Área Común', icon: '🏛️', placeholder: 'Seleccione área' },
  infraestructura: { label: 'Infraestructura', icon: '🏗️', placeholder: 'Seleccione sistema' }
};
...
const INFRASTRUCTURE = [
  ...
  { value: 'sistema_reciclado', label: 'Sistema de Reciclado de Agua', icon: '♻️' },
  ...
  { value: 'alarma_incendios', label: 'Sistema de Alarma contra Incendios', icon: '🚨' },
...
export default function Maintenance({ currentUser }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const areasData = areasSnap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  const handleViewHistory = async (location) => {
    ...
    try {
      ...
      try {
        ...
        const history = snap2.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        // Ordenar manualmente
        history.sort((a, b) => {
  ...
  // PERMISOS
  // ═══════════════════════════════════════════════════════════════════════════
  const canCreate = currentUser?.role === 'admin' || currentUser?.role === 'receptionist';
  ...
  return (
    <div className="maintenance-root">
      ...
      <header className="maintenance-header">
        <div>
          <h1><FaTools /> Gestión de Mantenimiento</h1>
                ...
                <div key={request.id} className={`request-card priority-${request.priority}`}>
                  <div className="request-header">
                    <div className="request-room">
                      {LOCATION_TYPES[request.locationType]?.icon || '📍'} {request.locationLabel || request.location}
                    </div>
        ...
        <div className="modal-overlay" onClick={handleCloseModal}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>
                <FaTools /> {editingRequest ? 'Editar' : 'Nueva'} Solicitud de Mantenimiento
              </h2>
            ...
            <div className="form-field">
                ...
                <select
                  ...
                  onChange={(e) => setForm({ ...form, locationType: e.target.value, location: '' })}
                >
              ...
              <div className="form-field">
                <label>
                  {form.locationType === 'habitacion' && 'Número de Habitación *'}
                  {form.locationType === 'area_comun' && 'Área Común *'}
                  {form.locationType === 'infraestructura' && 'Sistema/Infraestructura *'}
                </label>
                ...
                {form.locationType === 'habitacion' ? (
                  ...
                  >
                    ...
                    {customAreas.filter(a => a.type === 'area_comun').length > 0 && (
                      <optgroup label="Áreas Personalizadas">
                        {customAreas.filter(a => a.type === 'area_comun').map(area => (
                  ...
                  >
                    ...
                    {customAreas.filter(a => a.type === 'infraestructura').length > 0 && (
                      <optgroup label="Infraestructura Personalizada">
                        {customAreas.filter(a => a.type === 'infraestructura').map(area => (
                          <option key={area.id} value={area.id}>
                            {area.icon} {area.name}
                          </option>
                        ))}
              ...
              <div className="modal-actions">
                ...
                >
                  {saving ? (
                    ...
                    <><FaCheck /> {editingRequest ? 'Actualizar' : 'Crear'} Solicitud</>
                  )}
                </button>
              </div>
                      ...
                      <div key={item.id} className="history-item">
                        ...
                        <div className="history-details">
                          ...
                          <div className="history-meta">
                            <span><FaUser /> Creado por: {item.createdBy}</span>
                            {item.assignedTo && (
                              <span><FaWrench /> Asignado a: {item.assignedTo}</span>
                            )}
                            <span>
