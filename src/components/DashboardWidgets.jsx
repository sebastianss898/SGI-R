...
import {
  ...
  doc, query, orderBy, limit, onSnapshot, serverTimestamp, where
} from 'firebase/firestore';
import {
  FaBoxOpen, FaMinus, FaSpinner, FaExclamationTriangle,
  FaCalculator, FaCopy, FaCheck, FaEraser, FaUser
} from 'react-icons/fa';
...
const ROLES_PERMITIDOS = ['housekeeper', 'maintenance', 'receptionist', 'admin', 'manager'];
...
export function InventoryWidget({ currentUser }) {
  ...
  useEffect(() => {
    ...
    const unsubEmp = onSnapshot(
      ...
      snap => {
        ...
        setEmpleados(todos.filter(e => ROLES_PERMITIDOS.includes(e.role)));
        setLoadingEmp(false);
      },
  ...
  useEffect(() => {
    const unsubMov = onSnapshot(
      query(collection(db, 'inventarioMovimientos'), orderBy('fecha', 'desc'), limit(12)),
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError('');
    if (!selItem) { setError('Selecciona un insumo'); return; }
    if (!selEmp)  { setError('Selecciona quién lo Utilizara elproducto'); return; }
    ...
    if (!item) { setError('Insumo no encontrado'); return; }
    ...
    try {
      ...
      setSelItem(''); setSelEmp(''); setQty(1); setObs('');
    } catch (err) {
      console.error(err);
  ...
  const isLow = selectedItem && selectedItem.stockActual <= selectedItem.stockMinimo;
  ...
  return (
    <div className="dw-card">
      <div className="dw-header">
        <div className="dw-icon dw-icon--purple"><FaBoxOpen /></div>
        <div>
      ...
      <div className="dw-body">
        <form className="dw-inv-form" onSubmit={handleSubmit}>
          ...
          <div className="dw-row">
            <select className="dw-select" value={selItem}
              onChange={e => { setSelItem(e.target.value); setError(''); }}>
              <option value="">Selecciona insumo…</option>
              {items.map(i => (
                <option key={i.id} value={i.id}>
                  {i.nombre} — {i.stockActual} {i.unidad}{i.stockActual <= i.stockMinimo ? ' ⚠' : ''}
                </option>
              ))}
            </select>
            <input className="dw-input dw-input--qty" type="number" min="1"
              max={selectedItem?.stockActual || 999} value={qty}
              onChange={e => setQty(Math.max(1, Number(e.target.value)))} />
          </div>

          <div className="dw-emp-wrap">
            <div className="dw-emp-label"><FaUser /> ¿Quién lo utilizara?</div>
            {loadingEmp ? (
              <div className="dw-emp-loading"><FaSpinner className="spinning" /> Cargando empleados…</div>
            ) : empleados.length === 0 ? (
              <div className="dw-emp-loading">No hay empleados activos registrados</div>
            ) : (
              <select className="dw-select dw-select--emp" value={selEmp}
                onChange={e => { setSelEmp(e.target.value); setError(''); }}>
          ...
          <input className="dw-input" type="text"
          ...
          {isLow && (
            <div className="dw-warn">
              ...
              Stock bajo: quedan {selectedItem.stockActual} {selectedItem.unidad}(s)
            </div>
          )}
        ...
        {history.filter(m => m.tipo === 'salida').length === 0 ? (
          ...
          <div className="dw-history">
            {history.filter(m => m.tipo === 'salida').map(m => (
              <div key={m.id} className="dw-hist-item">
                ...
                <div className="dw-hist-info">
                  ...
                  <span className="dw-hist-meta">
                    ...
                    {m.observacion && m.observacion !== 'Uso general' ? ` · ${m.observacion}` : ''}
                    {' · '}{fmtTime(m.fecha)}
                  </span>
                </div>
              </div>
...
export function IvaWidget() {
  ...
  return (
    <div className="dw-card">
      <div className="dw-header">
        <div className="dw-icon dw-icon--green"><FaCalculator /></div>
        <div>
      ...
      <div className="dw-body">
        <div className="dw-iva-field">
          ...
          <div className="dw-iva-input-wrap">
            ...
            <input ref={inputRef} className="dw-iva-input" type="text"
              inputMode="numeric" placeholder="0" value={total} onChange={handleInput} />
            {total && (
              <button className="dw-iva-clear" onClick={() => { setTotal(''); inputRef.current?.focus(); }}>
                <FaEraser />
              </button>
            )}
          </div>
        ...
        <div className="dw-iva-results">
          <div className="dw-iva-result dw-iva-result--base">
            ...
            <span className="dw-iva-result-val">{hasVal ? fmtCOP(base) : '—'}</span>
          </div>
          <div className="dw-iva-result dw-iva-result--iva">
            <span className="dw-iva-result-label">Valor del IVA (19%)</span>
            <span className="dw-iva-result-val">{hasVal ? fmtCOP(iva) : '—'}</span>
          </div>
        </div>

        <div className="dw-iva-formula">
          <span className="dw-iva-formula-label">Fórmula:</span>
          <code>B = {hasVal ? fmtCOP(T) : 'T'} ÷ 1.19{hasVal ? ` = ${fmtCOP(base)}` : ''}</code>
        </div>

        {hasVal && (
          <div className="dw-iva-copy-row">
            <button className={`dw-iva-copy-btn ${copiedBase ? 'copied' : ''}`}
              onClick={() => copy(base, setCopiedBase)}>
              {copiedBase ? <><FaCheck /> Copiado</> : <><FaCopy /> Copiar base</>}
            </button>
            <button className={`dw-iva-copy-btn ${copiedIva ? 'copied' : ''}`}
              onClick={() => copy(iva, setCopiedIva)}>
              {copiedIva ? <><FaCheck /> Copiado</> : <><FaCopy /> Copiar IVA</>}
            </button>
          </div>
        )}

        <p className="dw-iva-tip">
          Fórmula: <strong>B = T / 1.19</strong> — aplica para IVA del 19% en Colombia.
          El total ya debe incluir el impuesto.
        </p>
      </div>
    </div>
  );
